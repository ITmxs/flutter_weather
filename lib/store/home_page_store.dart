...
import 'package:d9l_weather/models/weathermodel.dart';
...
abstract class HomePageBase with Store {
  ...
  @observable
  Now now;
  NowWeatherModel nowweather;
  String status;
  String cid;
  @observable
  ObservableList<DailyForecast> dailyForecastList =
    ...
    if (status != null) {
      if (status.contains('permission')) {
        ...
        Fluttertoast.showToast(
            msg: D9l.toastStr[D9l().lang]['no_more_requests']);
        _setEmptyData();
        return false;
      }
...
//如今最新的天气接口
  String basurl = "https://devapi.qweather.com/v7/weather/now";
  String key = "48fbadf80bbc43ce853ab9a92408373e";
  void _getweatherDatas() async {
    var resbody = await Dio().get(
      "${basurl}?location=121.27,30.19&key=${key}",
    );

    if (status != null) {
      nowweather = NowWeatherModel.fromJson(resbody.data["now"]);
      
      print(nowweather.cloud);
      if (nowweather.icon == 100) {
        print('100000000000000000000000');
      }
      print(nowweather.icon);
  ...
  Future _getLifeStyle() async {
    var result = await Http()
        .get('/weather/lifestyle?location=$cid&key=${Http.key}', null);
  ...
  Future _getAirQuality() async {
    ...
    if (result != null) {
      ...
      if (status == 'ok') {
        airQuality =
            AirQuality.fromJson(result['HeWeather6'].first['air_now_city']);
      }
    }
  }
}
